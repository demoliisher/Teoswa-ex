(function () {
    ...
    var VIEW_BOX = null;
    var MARGIN = 6;
    var MAP_URL = 'map.svg';   // sibling deliverable asset, fetch-injected
    var RAIL_URL = 'assets/data/rail-data.json';   // the rail tables (data, not a script)
    var LEGEND_URL = 'assets/legend.svg';   // the legend: drawer source + shot artwork
    ...
    // Under NARROW_Q the page replaces the in-SVG title/legend with HTML chrome
    // (top bar + collapsible legend drawer) and crops the map viewBox to the
    // drawing itself. The SVG keeps its own header/legend: they are hidden by an
    // external stylesheet rule, so they never enter svg.innerHTML, and the PNG
    // export still renders the untouched desktop layout (its root viewBox comes
    // from VIEW_BOX, captured before any zoom) — zooming mutates the live root
    ...
    var drawerLabel = drawer.querySelector('.drawer-label');
    ...
    // Taiwan-calligraphy woff2 subsets: the page gets them from app.css
    // (relative urls); the PNG export needs them re-embedded as base64 data URIs
    // because the exported SVG snapshot is rendered as an image, which may not
    // fetch any external resource. fetch() resolves the woff2 relative to the
    // page (works under http(s) and local file:// alike); when it fails, the
    // export silently falls back to the system font stack.
    var FONT_FACES = [
    ...
    // dist/ ships index.html (shell only) next to map.svg; boot() fetches the
    // drawing — plus the legend fragment and the rail tables — and injects it, which
    // is why the page must be served over http(s). Only the drawing is load-bearing:
    ...
    function boot() {
        if (!window.fetch) { failMap('fetch 不可用'); return; }
    ...
    function failMap(reason) {
        ...
        msg.id = 'map-error';
        msg.textContent = '地圖加載失敗' + (reason ? '（' + reason + '）' : '') +
            '：請通過 http(s) 訪問本站，或以 Chrome/Edge 打開本地文件。';
    ...
    function initMap(el) {
        ...
        // title and legend stay in svg.innerHTML for the export but reserve no
        // screen room, so the drawing sits centred.
    ...
    // filled by boot() from assets/data/rail-data.json (empty until then — nothing
    ...
    // 即是标记）。沒經過 is stored by *deleting* the key, so a value left behind by an
    // older build (or a hand edit — the sites map lives in localStorage) must not read
    // as "marked" either. Every judgement goes through here, so the main-line test and
    // the tail test can never drift apart again: a truthy check used to light a leg
    // whose two ends both held a stale "none"; a two-state whitelist would in turn have
    // dropped the 換乘 tier out of the railway lighting.
    ...
    // the legs to assets/data/rail-data.json **in that same cut order**, and the two
    ...
    function paintRail() {
        railSegsNodes.forEach(function (g, i) {
            ...
            // ship one (map_ex.rail.assert_tails_switched)
    ...
    function readRailLegs() {
        ...
        // 纯手动标记的站（机场）：构建期从表里推导，见 map_ex.rail.inference_exempt
        railBypass = {};
    ...
    function buildChooser() {
        ...
        none.textContent = clear ? clear.label : '沒經過';
    ...
    function renderNote(box) {
        ...
        home.textContent = '屬地：' + info.city + info.county;
        ...
        info.sites.forEach(function (s) {
            ...
            if (s.platforms && s.tracks) {
                ...
                sc.textContent = '（' + s.platforms + '台' + s.tracks + '線）';
                line.appendChild(sc);
            }
    ...
    // assets/data/rail-data.json (the map SVG itself carries no station
    ...
    // map_ex.rail.inference_exempt). A mark on the airport says the user reached the
    ...
    // The whole canvas, burnt-in title and legend included: only ever a fallback
    // (nothing to measure), because on screen both layouts frame the counties —
    // the export still serialises the full canvas, and it takes its size from
    // VIEW_BOX, not from whatever the element's viewBox happens to be.
    function fullVB() {
    ...
    // / legend column reserves no screen space (they stay in svg.innerHTML for the
    // export). The toggleable line overlays are excluded on
    ...
    function onPointerUp(e) {
        ...
        // Hover is off under narrow, so a mouse in a merely narrowed window used to
        // fall between the two paths and hit nothing at all: this branch now takes
        // it, and the mouse leaves before the touch double-tap below.
        if (narrow) {
    ...
    function bindView() {
        ...
        // Narrow: the pane is a flex item, so opening the legend drawer resizes it
        // and the drawing re-centres through **pure CSS** — no applyView() runs, so
        // nothing else would re-place the reticle and it would be left hanging
        // where its marker used to be.
    ...
    function buildChrome() {
        ...
        // the drawer is the page's whole control panel now, not just the legend
        drawerLabel.textContent = '控制面板';
        buildDrawer();
        var saved = null;
    ...
    function sizeDrawer() {
        ...
        // The measured number is the *unfolded* panel's, scrollbar and all — app.css
        // reserves that lane with `scrollbar-gutter: stable` so folding the section
        // back no longer shrinks the box, and this pin covers engines that do not
    ...
    function enterNarrow() {
        ...
        // xMidYMid): the pane spans exactly the free area between the top bar and
        // the legend drawer, so opening/closing the drawer re-centres the map with
        // even margins — no touching the title/score or the drawer.
    ...
    function levelsKey() {
        return getLevels().join('');
    }
    function saveLevels() {
        try { localStorage.setItem(STORE_KEY, levelsKey()); } catch (e) {}
    }
    function applyLevels() {
        var s = '';
        try { s = localStorage.getItem(STORE_KEY) || ''; } catch (e) {}
        regionPaths.forEach(function (p, i) {
            p.setAttribute('level', s[i] || '0');
        });
    }
    function updateScore() {
        ...
        scoreText.textContent = '分數：' + (sum + siteSum);
    ...
    // baked by svg/builder.py: the rows already sit in the shot's single column at the
    ...
    // The page's terrain wash, repainted on the export canvas: the same
    // corner-to-corner ramp app.css `html` paints (pale green → white → pale sea →
    // deeper blue).
    //
    // Painted as *iso-line* stripes rather than one `createLinearGradient`: Skia
    // dithers a smooth gradient to hide 8-bit banding, and that ±1-level noise is
    // unpredictable to PNG's Sub/Up/Paeth filters, so the whole canvas ends up
    // coding at ~1 byte per pixel. Measured on the 2x shot: 3,131,610 B dithered
    // vs 1,083,165 B for those very colours as stripes (688 KB of which is the
    // map). The stripes run perpendicular to the ramp — where the colour is
    // constant anyway — so the steps land exactly where a smooth gradient has its
    // own 1-level steps; at 128 of them the shot is visually the same picture.
    var WASH_STOPS = [[0, 0xC6, 0xE0, 0xBE], [0.22, 0xEA, 0xF3, 0xE6], [0.42, 0xFF, 0xFF, 0xFF],
                      [0.72, 0xA9, 0xCE, 0xE8], [1, 0x8C, 0xB6, 0xDA]];
    var WASH_BANDS = 128;
    function washColor(t) {
        var k = 0;
        while (k < WASH_STOPS.length - 2 && t > WASH_STOPS[k + 1][0]) k++;
        var lo = WASH_STOPS[k], hi = WASH_STOPS[k + 1];
        var f = (t - lo[0]) / Math.max(1e-6, hi[0] - lo[0]);
        return 'rgb(' + [1, 2, 3].map(function (j) {
            return Math.round(lo[j] + f * (hi[j] - lo[j]));
        }).join(',') + ')';
    }
    function paintWash(ctx, w, h) {
        var span = Math.sqrt(w * w + h * h);
        ctx.save();
        ctx.translate(w / 2, h / 2);
        ctx.rotate(Math.atan2(h, w));   // x' now runs along the ramp
        for (var i = 0; i < WASH_BANDS; i++) {
            ctx.fillStyle = washColor((i + 0.5) / WASH_BANDS);
            ctx.fillRect(-span / 2 + i * span / WASH_BANDS, -span, span / WASH_BANDS + 1, 2 * span);
        }
        ctx.restore();
    }
    ...
    function saveImage() {
        ...
        exportFonts.then(function (fontCss) {
            ...
            img.onload = function () {
                ...
                // the same corner-to-corner terrain wash the page paints with CSS
                // (app.css `html`): pale green → white → pale sea → deeper blue.
                // A canvas has no CSS, so the stops are repeated here; screen and
                // export are allowed to differ slightly.
                paintWash(ctx, canvas.width, canvas.height);
    ...
    function resetAll() {
        ...
        if (!window.confirm('確定要重置所有區縣標記嗎？')) return;
        try { localStorage.removeItem(STORE_KEY); } catch (e) {}
        try { localStorage.removeItem(siteStateKey()); } catch (e) {}
        try { localStorage.removeItem(RAIL_OFF_KEY); } catch (e) {}
        try { localStorage.removeItem(RAIL_SPECIAL_KEY); } catch (e) {}
        railOff = {};
        buildRailSwitches();   // 鐵路特例 开关也归零
